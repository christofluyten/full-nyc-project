...
class GraphPruner
{
    ...
    private GraphPruner(Map<String, Link> linkMap) {
...
//        System.out.println(count +" links deleted");
    ...
    static Map<String, Link> deleteUnvisitedLinks(Map<String, Link> linkMap) throws IOException, ClassNotFoundException {
        GraphPruner g1 = new GraphPruner(linkMap);
        g1.isSC(g1.adj);
        GraphPruner g2 = new GraphPruner(g1.deleteUnreachableNodes());
        g2.isSC(g2.adjTrans);
//        g2.deleteUnreachableNodes();
//        for(String id : g1.restMap.keySet()){
//            g2.restMap.put(id,g1.restMap.get(id));
//        }
//        System.out.println("g2.restMap.size "+g2.restMap.size()); //1554
//        return g2.restMap;
        Map<String, Link> result = g2.deleteUnreachableNodes();
                g2.deleteUnreachableNodes();
        for(String id : g1.restMap.keySet()){
            g2.restMap.put(id,g1.restMap.get(id));
        }
        System.out.println();
        System.out.println("total number of links = "+linkMap.size());
        System.out.println("number of links deleted = "+g2.restMap.size());
        System.out.println("graph contains now "+result.size()+" links");
        System.out.println();
        return result;
