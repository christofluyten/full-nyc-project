...
import static com.github.rinde.rinsim.geom.Point.distanceLonLat;
...
//      System.out.println("parcel "+parcel.toString()+" " +parcel.getPickupLocation());
...
//      System.out.println("commDistance "+commDistance+" amount of bidders "+result.size());
    ...
    private List<Bidder<T>> filterBidders(List<Bidder<T>> bidders, Parcel parcel, double commDistance ){
      ...
      for (final Bidder<T> bidder : bidders) {
        if (bidder != null && distanceLonLat(bidder.getBidderLocation(),parcel.getPickupLocation()) < commDistance) {
          result.add(bidder);
        }
      }
      return result;
...
//          for(Bid b:bids){
//            System.out.println("bid at end of auction "+b);
//
//          }
          final T winningBid = Collections.min(bids);
//          System.out.println("winner is " +winningBid);
